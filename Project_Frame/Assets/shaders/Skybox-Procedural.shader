...
Shader "KTGame/Skybox/Procedural" {
Properties {
    [KeywordEnum(None, Simple, High Quality)] _SunDisk ("Sun", Int) = 2
    _SunSize ("Sun Size", Range(0,1)) = 0.04
	_SunLit("Sun Lit", Range(0,3)) = 1
	_GroundMove("地平线偏移", Range(-0.3,0.3)) = 0
	_CloudRotateSpeed("云旋转速度", Range(-0.3,0.3)) = 0
    _SunSizeConvergence("Sun Size Convergence", Range(1,10)) = 5

    _AtmosphereThickness ("Atmosphere Thickness", Range(0,5)) = 1.0
    _SkyTint ("Sky Tint", Color) = (.5, .5, .5, 1)
    _GroundColor ("Ground", Color) = (.369, .349, .341, 1)
	_CloudSampler("云贴图", 2D) = "white" {}
...
SubShader {
    ...
    Pass {
        ...
        uniform half _GroundMove;
        uniform half _CloudRotateSpeed;
        uniform half _SunSize;
        uniform half _SunLit;
        ...
        v2f vert (appdata_t v)
        {
			...
			OUT.dir = normalize(v.vertex.xyz);
			...
			v.vertex.y += _GroundMove;

			float offsetValue = _CloudRotateSpeed * _Time.y + unity_DeltaTime.z;
			float3 t = RotateAroundYInDegrees(OUT.dir.xyz, offsetValue).xyz; //  animate rotation 
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            half lightColorIntensity = clamp(length(_LightColor0.xyz), 0.25, 1);
                ...
                OUT.sunColor    = kSimpleSundiskIntensityFactor * saturate(cOut * kSunScale) * _LightColor0.xyz / lightColorIntensity * _SunLit;
            #else // SKYBOX_SUNDISK_HQ
                OUT.sunColor    = kHDSundiskIntensityFactor * saturate(cOut) * _LightColor0.xyz / lightColorIntensity * _SunLit;
            #endif
        ...
        #endif
			col = min(1.1, col);
			SkyboxFog(IN.dir, col);
			half4 c = half4(col, 1.0);
            return c;
